...
- ReadableやAcrobatなどで日本語翻訳されたPDFは、本文の言語やファイル名の先頭・末尾にある明示的な`[ja]`・`_ja`・`jpn`・`日本語`・`日本語訳`・`翻訳`を検出し、ファイル名の先頭に`[ja] `を付けて原文PDFと区別します。タイトル本文中の`Japanese`・`translated`・`translation`は翻訳印として扱いません。ファイル名に自分で付けた`[ja]`は、DOI照合でもタイトル照合でも維持し、著者名として読み取りません。すでに印が付いている場合は二重に付けません。
...
Windows版はタスクトレーに常駐します。緑のアイコンは作動中、灰色は一時停止中です。右クリックメニューから「作動中（監視を再開）」「一時停止」「画面を開く」「終了」を選べます。ブラウザ画面を閉じても監視は続きます。

画面はWebView2を直接使い、ブラウザとは別の「論文PDFファイル名整理」専用ウィンドウで開きます。専用ウィンドウの×を押すとタスクトレーへ格納され、トレイアイコンのダブルクリックまたは「画面を開く」で再表示できます。「終了」を選んだときだけ監視を含むアプリ全体が終了します。

GitHub Releases版はZIPを展開し、最上位の`インストール.cmd`をダブルクリックしてください。Pythonや開発用`.venv`は不要です。ユーザー別の`%LOCALAPPDATA%\Programs\PaperPdfRenamer`へインストールするため、通常は管理者権限も不要です。WebView2 RuntimeがないPCでは、インストール時にMicrosoft公式Bootstrapperからユーザー別に導入します。
