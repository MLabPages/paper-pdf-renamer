...
- 2段組などで内部の文字順が崩れていても、1ページ目の文字座標と大きさから見出しを復元してタイトル照合に使います。
- DOIは画面に表示された文字ブロックを優先し、PDF内部のXML断片が末尾に混ざった場合は除去してから照合します。
- 著者名は補助照合です。DOI・タイトル・掲載情報が一致する場合、PDF本文からの著者抽出が不一致でも警告として表示し、候補を止めません。
- DOIがない場合は、タイトル検索に加えてファイル名やPDF本文から得た巻号・ページの一致も使います。主根拠が不足・矛盾する場合だけ `held`（要確認）にします。
- Crossrefで見つからないDOIなし論文は、OpenAlexでタイトル・第一著者・年の3点が一致した場合だけ補助照合として採用します。外部へ送るのはタイトル文字列だけです。
...
- 履歴には書誌情報も保存するため、変更済みPDFは再解析せずに「履歴から再整理」できます。過去の `held`（要確認）PDFは、現在の場所に残っていれば同じ操作から最新の抽出・照合方法で再スキャンします。
- 監視停止中に追加されたPDFは、前回監視時の一覧との差分として「要確認候補」にし、再起動直後に自動リネームしません。
- 英語著者は1名なら姓、2名なら`姓 & 姓`、3名以上なら`姓 et al.`と表記します。日本語著者は1名なら姓、2名なら`姓・姓`、3名以上なら`姓ほか`と表記します。
- ReadableやAcrobatなどで日本語翻訳されたPDFは、本文の言語やファイル名の`ja`・`日本語`・`翻訳`などを検出し、ファイル名の先頭に`[ja] `を付けて原文PDFと区別します。ファイル名に自分で付けた`[ja]`は、DOI照合でもタイトル照合でも維持し、著者名として読み取りません。すでに印が付いている場合は二重に付けません。
...
- このソフトで過去に変更したPDFの形式を直す場合：新しい`ファイル名形式`を保存 → `履歴から再整理／要確認を再スキャン（変更なし）` → 変更前後を確認・選択 → `チェックした候補をリネーム`。変更済みPDFは履歴に保存済みの情報を使い、過去の要確認PDFは最新の抽出・照合方法で再確認します。どちらも確認・実行まではファイル名を変更しません。
- 「要確認」「失敗」の行は自動では変更しません。元のファイル名を残したまま、理由を確認してください。
- 「候補」行に著者照合の警告が表示される場合があります。DOI・タイトル等の主根拠が一致していれば、著者名を手作業で確認しなくても候補を実行できます。
- PDFにDOIが直接書かれていない場合も、1ページ目の折り返しタイトルを文字座標と大きさから復元してCrossrefのタイトル検索を試みます。著者の所属番号や`et al.`は照合時に除外します。
- `ファイル名形式`には`{author}`、`{year}`、`{title}`、`{doi}`を使えます。例：`{author} ({year}). - {title}.pdf`。`.pdf`を省略した場合は自動で追加されます。英語2名の著者は`&`、3名以上は`et al.`、日本語2名は`・`、3名以上は`ほか`になります。
