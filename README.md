...
画面はWindows標準Edgeのアプリモードを使い、タブやアドレスバーのない「論文PDFファイル名整理」専用ウィンドウで開きます。専用ウィンドウを閉じてもアプリは終了せず、タスクトレーから再表示できます。

GitHub Releases版はZIPを展開し、最上位の`インストール.cmd`をダブルクリックしてください。Pythonや開発用`.venv`は不要です。ユーザー別の`%LOCALAPPDATA%\Programs\PaperPdfRenamer`へインストールするため、通常は管理者権限も不要です。
